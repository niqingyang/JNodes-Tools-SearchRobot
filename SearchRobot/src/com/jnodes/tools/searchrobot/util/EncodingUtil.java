...
public class EncodingUtil {
	...
	private static BytesEncodingDetect encodingDetect = new BytesEncodingDetect();
	...
	public static String detectEncoding(byte[] bytes){
		try {
			int index = encodingDetect.detectEncoding(bytes);
			String charset = BytesEncodingDetect.htmlname[index];
			return charset;
		} catch (Exception e) {
			return null;
		}
	...
	/**
	 * 自动识别byte[]的字符编码，转换为字符串
	 * @param bytes
	 * @return
	 */
	public static String toString(byte[] bytes) {
		String charset = EncodingUtil.detectEncoding(bytes);
		try {
			if(charset != null){
...
/**
	  public static void main(String argc1[]) {
	    BytesEncodingDetect sinodetector;
	    int result = OTHER;
	    int i;
	    sinodetector = new BytesEncodingDetect();
	    String argc="http://192.168.70.33/sp/ds.do?a=&b=1&c=10000&d=1000&e=1001&f=13800210500&g=0&h=&i=&j=1&k=46000";
//	    String argc="http://127.0.0.1/Test/t";
//	    for (i = 0; i < argc.length; i++) {
	      if (argc.startsWith("http://") == true) {
	        try {
	          result = sinodetector.detectEncoding(new URL(argc));
	        } catch (Exception e) {
	          System.err.println("Bad URL " + e.toString());
	        }
	      } else if (argc.equals("-d")) {
	        sinodetector.debug = true;
	      } else {
	        result = sinodetector.detectEncoding(new File(argc));
	      }
